...
const { shouldAbortOnEscape, useGlobalKeyboardShortcuts } = await jiti.import(
...
test("Cmd/Ctrl+K opens a new chat in the active project and respects handled events", async () => {
  ...
  const render = (activeCwd) =>
    React.act(() =>
      root.render(
        React.createElement(Shortcuts, {
          activeCwd,
          onNewSession: (cwd) => opened.push(cwd),
        }),
      ),
    );
  ...
  try {
    ...
    assert.equal(
      (await press({ ctrlKey: true, key: "K" })).defaultPrevented,
      true,
    );
    assert.deepEqual(opened, ["/tmp/project", "/tmp/project"]);
    assert.equal(
      (await press({ ctrlKey: true, repeat: true })).defaultPrevented,
      true,
    );
    ...
    for (const options of [
      ...
      { ctrlKey: true, altKey: true, key: "n" },
    ])
...
test("aborts the running turn on a plain Esc", () => {
  assert.equal(
    shouldAbortOnEscape({ defaultPrevented: false, tagName: "DIV" }),
    true,
  );
});
...
test("leaves the turn running when an overlay already handled Esc", () => {
  assert.equal(
    shouldAbortOnEscape({ defaultPrevented: true, tagName: "BUTTON" }),
    false,
  );
});
...
test("leaves the turn running for Esc on a dialog backdrop that handled it", () => {
  assert.equal(
    shouldAbortOnEscape({ defaultPrevented: true, tagName: "DIALOG" }),
    false,
  );
});
...
test("lets a textarea handle Esc itself", () => {
  assert.equal(
    shouldAbortOnEscape({ defaultPrevented: false, tagName: "TEXTAREA" }),
    false,
  );
});
...
test("lets an input handle Esc itself", () => {
  assert.equal(
    shouldAbortOnEscape({ defaultPrevented: false, tagName: "INPUT" }),
    false,
  );
});
...
test("tolerates an event with no element target", () => {
  assert.equal(
    shouldAbortOnEscape({ defaultPrevented: false, tagName: undefined }),
    true,
  );
});
