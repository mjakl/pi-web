...
const { shouldUseVisualViewportHeight, useViewportHeight } = await jiti.import(
...
test("normalizes the keyboard opening shift without fighting later viewport movement", async () => {
  ...
  try {
    await React.act(() =>
      root.render(React.createElement(ViewportHeightHarness)),
    );
    ...
    assert.equal(
      document.documentElement.style.getPropertyValue("--app-safe-area-bottom"),
      "0px",
    );
