...
function installHostPiRuntime(serialized = process.env.PI_WEB_HOST_PI) {
  ...
  if (!serialized)
    serialized = process.env.PI_WEB_HOST_PI = JSON.stringify(resolveHostPi());
  const runtime = JSON.parse(serialized);

  registerHooks({
    resolve(specifier, context, nextResolve) {
      const requested = specifier.replace(TURBOPACK_ALIAS, "");
      const packageName = Object.keys(runtime.packages).find(
        (name) => requested === name || requested.startsWith(`${name}/`),
      );
      ...
      if (requested === packageName) {
        return {
          shortCircuit: true,
          url: pathToFileURL(descriptor.entry).href,
        };
      }
      const target = exportedTarget(
        descriptor,
        requested.slice(packageName.length + 1),
      );
      if (!target || !target.startsWith("./"))
        throw new Error(
          `${specifier} is not exported by the validated host ${packageName} package.`,
        );
      let entry;
      try {
        ...
        throw new Error(
          `Validated host entry for ${specifier} does not exist.`,
        );
      }
      ...
      if (
        ...
        throw new Error(
          `Validated host entry for ${specifier} leaves ${packageName}.`,
        );
      }
