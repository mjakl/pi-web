...
test("test-one forwards a spaced name pattern and file path through the host preload", (t) => {
  ...
  writeFileSync(
    path.join(dir, "unselected.test.mjs"),
    'throw new Error("file selection was lost");',
  );
  const result = just([
    "test-one",
    "--test-name-pattern",
...
test("test-one propagates test failure and rejects missing selection", (t) => {
  ...
  writeFileSync(
    failing,
    'import test from "node:test"; test("intentional failure", () => { throw new Error("failure reached caller"); });',
  );
  ...
  for (const failure of [null, "lint", "typecheck", "test"]) {
    test(`${recipe} validates without fixing and stops at ${failure ?? "successful completion"}`, (t) => {
      ...
      writeFileSync(
        ...
        JSON.stringify({ scripts: fixtureScripts }),
      );
      writeFileSync(
        path.join(dir, "step.cjs"),
        `
      ...
      const steps = readFileSync(path.join(dir, "steps.log"), "utf8")
        .trim()
        .split("\n");
