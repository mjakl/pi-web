...
test("groups discover external folders on expansion, select paths, and refresh on re-expansion", async (t) => {
  ...
  await act(() =>
    root.render(
      React.createElement(ProjectFolderGroup, {
  ...
  assert.equal(
    container.querySelector('[aria-current="true"]').title,
    paths[0],
  );
  assert.equal(container.textContent.includes("Main folder"), false);
  await act(() =>
    container.querySelectorAll(".project-folder-child")[1].click(),
  );
