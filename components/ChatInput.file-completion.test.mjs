...
test("path completion ignores late responses and inserts an absolute file mention", async () => {
  ...
  try {
    ...
    await React.act(() =>
      pending[1].resolve(
        Response.json({ matches: [{ path: "/second/b.txt", isDir: false }] }),
      ),
    );
    await React.act(() =>
      pending[0].resolve(
        Response.json({ matches: [{ path: "/first/a.txt", isDir: false }] }),
      ),
    );
    assert.equal(container.textContent.includes("/first/"), false);
    const file = [...container.querySelectorAll("button")].find((button) =>
      button.textContent.includes("b.txt"),
    );
    assert.ok(file);
    await React.act(() =>
      file.dispatchEvent(
    ...
    await type("@/missing/");
    await React.act(() =>
      pending[2].resolve(new Response(null, { status: 404 })),
    );
    ...
    await React.act(() =>
      pending[3].resolve(
        Response.json({ files: ["report.txt"], truncated: false }),
      ),
    );
    const projectFile = [...container.querySelectorAll("button")].find(
      (button) => button.textContent === "report.txt",
    );
    assert.ok(projectFile);
    await React.act(() =>
      projectFile.dispatchEvent(
        new window.MouseEvent("mousedown", { bubbles: true, cancelable: true }),
      ),
    );
