...
const { shouldAbortOnEscape } = await jiti.import(
...
for (const request of [
  ...
  test(`Escape cancels a ${request.method} request instead of aborting the turn`, async () => {
    const { responses, aborts } = await withDialog(
      request,
      (dialog, responses) => {
    ...
    assert.equal(
      aborts,
      false,
      "Escape must not reach the global abort shortcut",
    );
  });
}
...
test("the request is a real modal, not a div wearing a dialog role", async () => {
  ...
  assert.equal(
    tag.open,
    true,
    "showModal() must have run so the focus trap is real",
  );
});
