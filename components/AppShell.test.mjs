  ...
  test(`context usage agrees between the top bar and statistics panel on desktop and mobile (estimated: ${estimated})`, async () => {
    ...
    try {
      for (const width of [1064, 390, 320]) {
        ...
        assert.ok(
          button.textContent.includes(`${mark}48k / 200k (${mark}24%)`),
        );
        assert.ok(
          button.title.includes(
            `Context: ${mark}48,000 / 200,000 tokens (${mark}24%)`,
          ),
        );
        ...
        assert.equal(
          valueFor("Context window"),
          `${mark}48,000 / 200,000 tokens`,
        );
        assert.equal(valueFor("Context usage"), `${mark}24%`);
        assert.equal(valueFor("Input"), "12,000");
        assert.equal(valueFor("Output"), "4,000");
      }
    } finally {
      await act(() => root.unmount());
      container.remove();
