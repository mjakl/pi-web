    ...
    return (
      <div className="subagent-body">
        ...
        {row && (
          <div className="subagent-result">
            ...
            {!row.output && !row.error && (
              <p>
                {t(
                  row.handledWithoutAgent
                    ? "chat.subagent.handled"
                    : "chat.subagent.noOutput",
                )}
              </p>
            )}
            {row.captureTruncated && (
              <p className="subagent-notice">
                {t("chat.subagent.captureTruncated")}
              </p>
            )}
          </div>
        )}
        <Disclosure
          ...
          label={t("chat.subagent.prompt")}
        >
          {() => <pre className="subagent-plain">{call.prompt}</pre>}
        </Disclosure>
        <Disclosure
          className="subagent-disclosure"
          label={t("chat.subagent.runDetails")}
        >
          {() => (
            <dl className="subagent-settings">
              <dt>{t("chat.subagent.agent")}</dt>
              <dd>{call.agent}</dd>
              {model && (
                <>
                  <dt>{t("chat.subagent.model")}</dt>
                  <dd>{model}</dd>
                </>
              )}
              {agentCwd && (
                <>
                  <dt>{t("chat.subagent.cwd")}</dt>
                  <dd>{agentCwd}</dd>
                </>
              )}
              {call.initialContext && (
                <>
                  <dt>{t("chat.subagent.initialContext")}</dt>
                  <dd>{call.initialContext}</dd>
                </>
              )}
              {call.session && (
                <>
                  <dt>{t("chat.subagent.session")}</dt>
                  <dd>{call.session}</dd>
                </>
              )}
            </dl>
    ...
    <Disclosure
      ...
      label={
        <span className="subagent-header">
          ...
          {counts ? (
            <span className="subagent-counts">{counts}</span>
          ) : (
          ...
          {running && activity?.progress && (
            <span className="subagent-progress">{activity.progress}</span>
          )}
        </span>
      }
    >
      {() => (
        <>
          {result && !rows && (
            <div className="subagent-body subagent-result">
              <div className="subagent-section-label">
                {t("chat.subagent.result")}
              </div>
              <SafeMarkdownBody
                ...
                sessionId={sessionId}
              >
                {rawOutput || t("chat.subagent.noOutput")}
              </SafeMarkdownBody>
            </div>
          )}
          {singleCall
          ...
          <div className="subagent-raw">
            <Disclosure
              className="subagent-disclosure"
              label={t("chat.subagent.rawInput")}
            >
              {() => (
                <pre className="subagent-plain">
            ...
            {result && (
              <Disclosure
                className="subagent-disclosure"
                label={t("chat.subagent.rawOutput")}
              >
