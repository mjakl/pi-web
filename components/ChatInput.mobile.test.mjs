...
test("Session controls is mobile-only and disappears when its menu has no actions", async () => {
  ...
  const more = () =>
    container.querySelector('button[aria-label="Session controls"]');
  try {
    ...
    await React.act(() =>
      root.render(
        React.createElement(ChatInput, { ...props, isStreaming: true }),
      ),
    );
    assert.equal(more(), null);
    await React.act(() =>
    ...
    assert.ok(more());
    assert.match(
      container.querySelector(".menu-composer-controls").textContent,
      /ponytail: FULL/,
    );
    await React.act(() =>
      root.render(
        React.createElement(ChatInput, { ...props, extensionStatuses: [] }),
      ),
    );
    assert.equal(more(), null);
    await React.act(() =>
    ...
    assert.ok(more());
    await React.act(() =>
      container.querySelector(".menu-composer-controls").dispatchEvent(
        Object.assign(new window.Event("toggle"), {
          newState: "open",
          oldState: "closed",
        }),
      ),
    ...
    await React.act(() =>
      root.render(
        React.createElement(ChatInput, { ...props, onModelChange: undefined }),
      ),
    );
    assert.equal(more(), null);
    assert.ok(
      container.querySelector("select"),
      "standalone reasoning stays accessible",
    );
    await React.act(() =>
      window.happyDOM.setWindowSize({ width: 390, height: 844 }),
    );
...
test("the combined model picker retains reasoning while More keeps Stop and extension status", async () => {
  ...
  try {
    ...
    await React.act(() =>
      container
        .querySelector('button[aria-label="Model and reasoning"]')
        .click(),
    );
    ...
    assert.equal(
      container.querySelector('button[aria-label="Session controls"]').disabled,
      false,
    );
    const menu = container.querySelector(".menu-composer-controls");
...
test("the mobile footer keeps status updates exposed while the menu provides a non-live copy", async () => {
  ...
  try {
    for (const widgets of [
      [],
      [{ key: "usage", lines: ["Ready"], placement: "aboveEditor" }],
    ]) {
      await React.act(() => root.render(render("Working", widgets)));
      const status = container.querySelector(
        ".chat-composer > .extension-status-shelf [role=status]",
      );
      assert.ok(status);
      ...
      assert.equal(
        container.querySelectorAll('.extension-status-shelf [role="status"]')
          .length,
        1,
      );
      assert.match(
