...
test(
  "Node 22.19 serves a persisted session through the packaged bin and host Pi",
  { timeout: 90_000 },
  async () => {
    assert.equal(process.versions.node, "22.19.0");
    assert.ok(
      process.env.PI_WEB_SMOKE_PACKAGE,
      "Set PI_WEB_SMOKE_PACKAGE to the installed package directory",
    );
    ...
    for (const modules of [
      ...
      for (const dependency of ["typescript", "@earendil-works"]) {
        await assert.rejects(access(path.join(modules, dependency)), {
          code: "ENOENT",
        });
      }
    }
    ...
    try {
      ...
      await writeFile(
        path.join(
          sessionDir,
          `${timestamp.replaceAll(":", "-")}_${sessionId}.jsonl`,
        ),
        [
          { type: "session", version: 3, id: sessionId, timestamp, cwd: home },
          {
            ...
            message: {
              role: "user",
              content: [{ type: "text", text: "Runtime smoke fixture" }],
      ...
      assert.deepEqual(body.context.messages[0].content, [
        { type: "text", text: "Runtime smoke fixture" },
      ]);
      assert.equal(body.filePath.startsWith(sessionDir + path.sep), true);
      console.log(
        `Packaged startup and host Pi session read passed on Node ${process.versions.node}`,
      );
    } finally {
