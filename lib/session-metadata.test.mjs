  ...
  await writeFile(
    ...
    [
      ...
      line({
        ...
        message: { role: "user", content: expandedSkill },
      }),
      line({
        type: "message",
        id: "a1",
...
test("rejects a stale inventory fingerprint without returning metadata", async (t) => {
  ...
  assert.equal(
    await readSessionRowMetadata(filePath, "session-1", {
      fileSize: 0,
...
test("stable reads return values and preserve callback errors only when identity is unchanged", async (t) => {
  ...
  await assert.rejects(
    readStableSessionFile(filePath, () => {
      throw callbackError;
    }),
...
test("stable reads reject changed identity after callback success or error", async (t) => {
  const changes = {
    ...
    metadata: (filePath) =>
      utimes(filePath, fixedTime, new Date("2026-01-02T00:00:00.000Z")),
  ...
  for (const [name, change] of Object.entries(changes)) {
    for (const callbackFails of [false, true]) {
      await t.test(
        `${name} after callback ${callbackFails ? "error" : "success"}`,
        async (t) => {
...
test("stable reads reject a file appearing after a transient read", async (t) => {
  for (const callbackFails of [false, true]) {
    await t.test(
      `after callback ${callbackFails ? "error" : "success"}`,
      async (t) => {
        const dir = await mkdtemp(
          join(tmpdir(), "pi-web-stable-read-appeared-"),
        );
        const filePath = join(dir, "session.jsonl");
        t.after(() => rm(dir, { recursive: true, force: true }));
        const result = await readStableSessionFile(filePath, async () => {
...
test("caps plain first-message titles", () => {
  const title = sessionTitleFromFirstMessage(
    "x".repeat(SESSION_TITLE_MAX_CHARS + 100),
  );
