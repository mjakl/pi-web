...
test("retained wrappers without isActive remain active across hot reload", async (t) => {
  const previousRegistry = globalThis.__piSessions;
  const id = "hot-reload-wrapper";
  const wrapper = {
    sessionId: id,
    isAlive: () => true,
  };
  ...
  assert.deepEqual(getActiveRpcSessionIds(), [id]);
  assert.deepEqual(
    await startRpcSession(id, "/path/must/not/be/opened.jsonl", undefined),
    { session: wrapper, realSessionId: id },
  );
});
