...
test("partial recall and new sends progress without restarting retained input", async (t) => {
  const calls = [];
  const { inner, wrapper, emit } = compactionSession(
    t,
    async (text, options) => {
  ...
  const recalled = await wrapper.send({
    type: "clear_queue",
    imageCapacity: 1,
  });
  await wrapper.send({
    type: "prompt",
    message: recalled.followUp[0],
    images: recalled.images,
  });
  ...
  await nextTurn();
  assert.deepEqual(calls, ["one", "new input"]);
  assert.deepEqual(
    (await wrapper.send({ type: "get_state" })).queuedMessages.followUp,
    ["two"],
  );
...
test("compaction preserves extension command dispatch and recalls only attachments that fit", async (t) => {
  const calls = [];
  ...
  assert.deepEqual(recalled.followUp, ["one"]);
  assert.deepEqual(recalled.images, images);
  assert.deepEqual(recalled.queuedMessages.followUp, ["two"]);
  assert.deepEqual(
    (await wrapper.send({ type: "clear_queue", imageCapacity: 0 }))
      .queuedMessages.followUp,
    ["two"],
  );
  assert.deepEqual(
    (await wrapper.send({ type: "clear_queue", imageCapacity: 1 })).images,
    images,
  );
});
