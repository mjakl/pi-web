...
test("tool-policy rebuilds serialize under the source lifecycle", async (t) => {
    ...
    inner: {
      sessionManager: {
        appendCustomEntry(_type, data) {
          appendedSelections.push(data.tools);
        },
      },
      model: undefined,
      agent: { state: {} },
  ...
  await nextTurn();
  const second = setRpcSessionTools(
    beginRpcSessionOperation(id),
    undefined,
    [],
  );
  await nextTurn();
  ...
  const stopping = stopRpcSession(id);
  finishShutdown();
  const [firstResult, secondResult, stopResult] = await Promise.allSettled([
    first,
    second,
    stopping,
  ]);
