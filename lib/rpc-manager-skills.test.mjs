...
async function createSkillSession(t, { responses = [] } = {}) {
  ...
  await writeSkill(
    discoveredSkills,
    "visible-skill",
    "Visible first-turn guidance",
    "Visible skill body.",
  );
  await writeSkill(
    discoveredSkills,
    "manual-skill",
    "Manual first-turn workflow",
    "Manual skill body.",
    true,
  );
...
test("a new session slash palette includes resources discovered during startup", async (t) => {
  ...
  const skills = result.commands.filter(
...
test("direct first-turn skill input matches later expansion and keeps model visibility rules", async (t) => {
  ...
  assert.equal(
    parsed.content,
    `References are relative to ${dirname(parsed.location)}.\n\nManual skill body.`,
  );
