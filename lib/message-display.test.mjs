...
test("expands process details for any non-empty assistant text before or between tool calls", async () => {
  ...
  const processMessages = [
    assistant([
      ...
      { type: "toolCall", toolCallId: "call-2", toolName: "read", input: {} },
    ]),
  ];

  assert.equal(
    shouldExpandProcessDetails(processMessages, { hasFinalAnswer: true }),
    true,
  );
});

test("expands process details for an assistant image", async () => {
  const { shouldExpandProcessDetails } = await loadSubject();
  const processMessages = [
    assistant([
      {
...
test("keeps tool, thinking, and empty text details collapsed when a final answer is visible", async () => {
  const { shouldExpandProcessDetails } = await loadSubject();
  const processMessages = [
    assistant([
      { type: "thinking", thinking: "work through it" },
...
test("expands process details when a completed turn has no final answer", async () => {
  const { shouldExpandProcessDetails } = await loadSubject();
  const processMessages = [
    assistant([
      { type: "toolCall", toolCallId: "call-1", toolName: "bash", input: {} },
  ...
  assert.equal(
    shouldExpandProcessDetails(processMessages, { hasFinalAnswer: false }),
    true,
  );
});
...
test("drops empty thinking and text blocks after completion", async () => {
...
test("keeps empty thinking and text while streaming", async () => {
  const { splitFinalAssistantBlocks } = await loadSubject();
  const message = assistant([
    { type: "thinking", thinking: "" },
    { type: "text", text: "" },
