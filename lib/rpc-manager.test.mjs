...
const { AgentSessionWrapper, startRpcSession } =
  await jiti.import("./rpc-manager.ts");

for (const type of ["fork", "clone"]) {
  test(`${type} keeps the source usable after creating independent children`, async () => {
    ...
    const forkEntryId = manager.appendMessage({
      role: "user",
    ...
    const command =
      type === "fork"
        ? { type, entryId: forkEntryId }
        : { type, leafId: cloneLeafId };
    ...
    try {
      ...
      assert.equal(
        wrapper.isActive(),
        true,
        "creating a child must leave the source active",
      );
      ...
      assert.equal(
        source.buildSessionContext().messages.at(-1).content,
        "continue the source",
      );
...
test("missing working folders block activation and mutation without rewriting history", async (t) => {
  ...
  await assert.rejects(
    startRpcSession(manager.getSessionId(), file, undefined),
    /read-only/,
  );
