...
test("RPC startup leaves extension tool names to the SDK", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  ...
  assert.match(activationSource, /session\s*\.getAllTools\(\)/);
  assert.match(activationSource, /filter\(\(name\) => !codingToolNames\.has\(name\)\)/);
  assert.match(startupSource, /extensionsOverride: preferUserBashExtension/);
  assert.doesNotMatch(startupSource, /excludeTools/);
  assert.doesNotMatch(activationSource, /"Agent"|"get_subagent_result"|"steer_subagent"/);
});
...
test("Stop fences startup registration and waits for the in-flight start", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const stopSource = source.slice(
    source.indexOf("export async function stopRpcSession"),
    source.indexOf("export async function startRpcSession"),
  );

  assert.match(startupSource, /operation\?: RpcSessionOperation/);
  assert.match(startupSource, /const startGeneration = operation\?\.generation \?\? lifecycle\.generation/);
  assert.match(startupSource, /lifecycle\.generation !== startGeneration/);
  assert.ok(
    startupSource.indexOf("generation !== startGeneration")
      < startupSource.indexOf("registerRpcWrapper(wrapper)"),
  );
  assert.match(stopSource, /lifecycle\.generation \+= 1/);
  assert.match(stopSource, /await locks\.get\(sessionId\)/);
  assert.match(stopSource, /await session\.shutdown\(\{ manual: true \}\)/);
  assert.match(startupSource, /locks\.set\(lifecycleSessionId, starting\)/);
});
...
test("normal session teardown paths use graceful extension shutdown", async () => {
  ...
  assert.match(deleteRouteSource, /await stopRpcSession\(id\)/);
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  const registrationSource = source.slice(
    ...
    source.indexOf("function getLocks"),
  );

  assert.match(startupSource, /readSessionToolSelection\([\s\S]*?sessionManager\.getEntries\(\)/);
  assert.match(startupSource, /const selectedToolNames = persistedToolNames \?\? requestedToolNames/);
  ...
  assert.match(startupSource, /const trustReloadOptions = chatOnly[\s\S]*?projectTrustReloadOptions\(sessionCwd, agentDir\)/);
...
test("crossing the Chat-only boundary persists and rebuilds the wrapper", async () => {
  ...
  const switchSource = source.slice(
    ...
    source.indexOf("export function getActiveRpcSessionIds"),
  );
