...
const { createWebPushNotifier } = await loadSubject();
const pushClientSource = await readFile(new URL("./push-client.ts", import.meta.url), "utf8");
const subscribeRouteSource = await readFile(new URL("../app/api/push/subscribe/route.ts", import.meta.url), "utf8");
...
function makeEnvironment({
  ...
  const env = {
    ...
    getSessionName: async (sessionId) => sessionNames.get(sessionId),
  };
...
test("new push subscriptions do not send or store a locale", () => {
  assert.match(pushClientSource, /JSON\.stringify\(\{ subscription: subscription\.toJSON\(\) \}\)/);
  assert.doesNotMatch(pushClientSource, /setupPushSubscription\(locale|subscription\.toJSON\(\), locale/);
  assert.doesNotMatch(subscribeRouteSource, /body\.locale|locale,/);
});
...
test("addSubscription upserts by endpoint and persists", () => {
  ...
  assert.deepEqual(updated.keys, { p256dh: "updated-p256dh", auth: "updated-auth" });
...
test("notifySessionComplete sends English payloads to new and locale-tagged subscriptions", async () => {
...
test("notifySessionComplete falls back to the English generic title", async () => {
