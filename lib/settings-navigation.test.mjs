...
test("restores the last settings section and falls back without a project", () => {
  ...
  setLastSettingsSection("general", storage);
  assert.equal(getLastSettingsSection("/project", storage), "general");
});

test("falls back when a removed section is still persisted", () => {
  const storage = createStorage({
    "pi-web:settings-navigation": JSON.stringify({ section: "models" }),
  });
