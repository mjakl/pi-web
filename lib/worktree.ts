...
export function assertWorkingDirectoryAvailable(cwd: string): void {
  if (!isWorkingDirectoryAvailable(cwd)) {
    throw new Error(
      `Working folder is unavailable. This session is read-only: ${cwd}`,
    );
  }
}
...
function readKnownProjects(): Record<string, string> {
  try {
    const value: unknown = JSON.parse(
      readFileSync(join(getAgentDir(), "web-worktree-projects.json"), "utf8"),
    );
...
function rememberProjects(paths: string[], root: string): void {
  ...
  writePrivateFileAtomicSync(
    join(getAgentDir(), "web-worktree-projects.json"),
...
function removedProject(cwd: string): ProjectInfo {
  ...
  return {
    ...
    branch: null,
    isWorktree: Boolean(root && !samePath(root, cwd)),
    isTopLevel: Boolean(root),
  };
}
  ...
  return records
    .filter(
      (record) =>
        !record.bare &&
        !record.prunable &&
        isWorkingDirectoryAvailable(record.path),
    )
...
export function findCurrentWorktreePath(
  ...
  return (
    worktrees.find((worktree) => samePath(worktree.path, realPathOrSelf(cwd)))
      ?.path ?? null
  );
}
