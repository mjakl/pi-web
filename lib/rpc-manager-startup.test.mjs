...
test("native PowerShell configuration remains active and untouched", async (t) => {
  const { agentDir, start } = await fixture(t, {
    defaultTools: ["read", "powershell", "edit", "write"],
  });
  const settingsPath = join(agentDir, "settings.json");
  const before = await readFile(settingsPath, "utf8");
  const wrapper = await start();
  assert.ok(wrapper.inner.getActiveToolNames().includes("powershell"));
