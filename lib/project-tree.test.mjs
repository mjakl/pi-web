...
const { MAX_PROJECTED_TREE_DEPTH, projectTreeForResponse } = await createJiti(
  import.meta.url,
).import("./project-tree.ts");
...
test("retained roots, branch points and leaves transfer only navigation fields", (t) => {
  // Deterministic high-entropy synthetic data, never a user's session content.
  const payload = createHash("shake256", { outputLength: 1024 * 1024 })
    .update("pi-web navigation-only tree regression")
    .digest("base64");
  const makeTree = (body) => [
    {
      ...node(msg("root", "user", [{ type: "image", data: body }]), [
        node(
          msg("branch", "assistant", [
            { type: "thinking", thinking: body },
            { type: "text", text: "Branch preview" },
            { type: "toolCall", arguments: { body } },
          ]),
          [
            node(
              msg("image-leaf", "toolResult", [{ type: "image", data: body }]),
            ),
            node({ id: "summary-leaf", type: "branch_summary", summary: body }),
            node({ id: "compaction-leaf", type: "compaction", summary: body }),
            node({ id: "custom-leaf", type: "custom", data: { body } }),
          ],
        ),
      ]),
      label: "Saved root",
  ...
  const source = makeTree(payload);
  const projected = projectTreeForResponse(source);
  const serialized = JSON.stringify(projected);
  t.diagnostic(
    `Synthetic projected tree: raw=${Buffer.byteLength(serialized)} gzip=${gzipSync(serialized).length} bytes`,
  );
  assert.ok(
    serialized === JSON.stringify(projectTreeForResponse(makeTree("small"))),
    "entry body size must not affect the navigation response",
  );
  ...
  assert.equal(projected[0].label, "Saved root");
  assert.deepEqual(projected[0].branchPreview, {
    role: "user",
    text: "[image]",
  });
  assert.equal(source[0].entry.message.content[0].data, payload);
});

test("retains each human message with its own preview and contracts non-message prefixes", () => {
  ...
  assert.equal(projectedA1.children[1].compressedEntryIds, undefined);
...
test("carries previews through non-message roots in multi-root trees", () => {
  ...
  assert.equal(projected[0].children[0].entry.id, "u1");
  assert.equal(projected[1].children[0].entry.id, "u2");
});
...
test("normalizes and bounds preview text and labels image-only messages", () => {
  ...
  const textPreview = findProjectedNode(projected, "u1").branchPreview;
  const imagePreview = findProjectedNode(projected, "u2").branchPreview;
...
test("carries previews through the depth-limit flattening path", () => {
  ...
  const leaf = findProjectedNode(projected, "deep-user");
  assert.deepEqual(leaf.branchPreview, { role: "user", text: "深层问题" });
  assert.deepEqual(leaf.compressedEntryIds, ["prefix"]);
  assert.equal(findProjectedNode(projected, "deep-leaf").parentId, "deep-user");
});
