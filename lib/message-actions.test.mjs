...
async function fixture(t) {
  ...
  const inner = {
    ...
    navigateTree(targetId, options) {
      return AgentSession.prototype.navigateTree.call(
        {
          ...inner,
          _extensionRunner: inner.extensionRunner,
...
test("rail navigation selects exact user and custom-message leaves without editing them", async (t) => {
  const { manager, agent, wrapper, custom, user, later, events } =
    await fixture(t);
  const metadata = manager.appendSessionInfo("Branch label");
  for (const targetId of [user, custom, later, metadata]) {
    const result = await wrapper.send({ type: "navigate_tree", targetId });
    assert.equal(manager.getLeafId(), targetId);
    assert.deepEqual(result, { cancelled: false, leafId: targetId });
    assert.deepEqual(
      agent.state.messages,
      manager.buildSessionContext().messages,
    );
    assert.equal(events.at(-1).newLeafId, targetId);
  }
  assert.equal(agent.state.messages.at(-1).content, "Later");
  const eventCount = events.length;
  assert.deepEqual(
    await wrapper.send({ type: "navigate_tree", targetId: metadata }),
    { cancelled: false, leafId: metadata },
  );
  assert.equal(
    events.length,
    eventCount,
    "the already-selected leaf is a no-op",
  );
});
test("rail navigation preserves the current branch on extension cancellation and rejects busy sessions", async (t) => {
  const { manager, agent, inner, wrapper, custom } = await fixture(t);
  const leaf = manager.getLeafId();
  const messages = agent.state.messages;
  inner.extensionRunner.emit = async (event) =>
    event.type === "session_before_tree" ? { cancel: true } : undefined;
  assert.deepEqual(
    await wrapper.send({ type: "navigate_tree", targetId: custom }),
    { cancelled: true },
  );
  assert.equal(manager.getLeafId(), leaf);
  assert.deepEqual(agent.state.messages, messages);
  for (const activity of ["isStreaming", "isBashRunning", "isCompacting"]) {
    ...
    await assert.rejects(
      wrapper.send({ type: "navigate_tree", targetId: custom }),
      /current|running/,
    );
...
for (const type of ["branch_from_message", "navigate_tree"]) {
  test(`${type} waits for extension cancellation and rejects conflicting commands`, async (t) => {
    const { manager, agent, inner, wrapper, custom } = await fixture(t);
    const leaf = manager.getLeafId();
    const messages = [...agent.state.messages];
    let release;
    ...
    const branch = wrapper.send({ type, entryId: custom, targetId: custom });
    await entered;
    await assert.rejects(
...
for (const type of ["branch_from_message", "navigate_tree"]) {
  test(`stopping while ${type} waits cancels navigation without changing context`, async (t) => {
    const { manager, agent, inner, wrapper, custom } = await fixture(t);
    const leaf = manager.getLeafId();
    const messages = [...agent.state.messages];
    let entered;
