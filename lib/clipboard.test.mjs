...
test("falls back to execCommand outside a secure context and resolves when it copies", async () => {
  ...
  assert.equal(
    ...
    null,
    "the scratch textarea must not leak",
  );
});
...
test("rejects when execCommand refuses, so callers cannot report a copy that did not happen", async () => {
  await withNavigator({}, () =>
    withExecCommand(
      () => false,
      () =>
        assert.rejects(copyText("hello"), {
          message: "The browser refused to copy to the clipboard",
        }),
    ),
  );
  assert.equal(
    document.querySelector("textarea"),
    null,
    "the scratch textarea must not leak on refusal",
  );
});
...
test("rejects with the thrown error when execCommand throws", async () => {
  ...
  assert.equal(
    document.querySelector("textarea"),
    null,
    "the scratch textarea must not leak when copying throws",
  );
});
