...
test("completes only immediate files and folders outside the project, including hidden entries and links", async (t) => {
  ...
  assert.equal(
    entries.find((entry) => entry.path.endsWith("linked")).isDir,
    true,
  );
  assert.equal(
    entries.find((entry) => entry.path.endsWith("broken")).isDir,
    false,
  );
  ...
  assert.deepEqual(
    await completeFilePath(path.join(root, "notes folder") + path.sep, project),
    [{ path: path.join(root, "notes folder", "nested.txt"), isDir: false }],
  );
  await assert.rejects(
    completeFilePath(path.join(root, "missing") + path.sep, project),
  );
});
...
test("filters the whole directory before limiting completion results", async (t) => {
  ...
  await Promise.all(
    Array.from({ length: 30 }, (_, i) =>
      writeFile(path.join(root, `file-${String(i).padStart(2, "0")}`), ""),
    ),
  );
  const { completeFilePath } = await loadSubject();
  assert.equal((await completeFilePath(root + path.sep)).length, 20);
  assert.deepEqual(await completeFilePath(path.join(root, "file-29")), [
    { path: path.join(root, "file-29"), isDir: false },
  ]);
});
