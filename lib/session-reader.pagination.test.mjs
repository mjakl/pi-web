...
test("history anchors cover unloaded and compacted turns only on the selected branch", () => {
  ...
  assert.equal(page.historyAnchorIds.length, 51);
  assert.equal(page.historyAnchorIds[0], "e0");
  assert.equal(page.historyAnchorIds.at(-1), "compact");
  assert.deepEqual(buildSessionContext(entries, "alternate", { tail: 1 }).historyAnchorIds, ["e0", "alternate"]);
...
test("a rail jump loads a contiguous older span through its target without crossing branches", () => {
  ...
  assert.equal(page.historyAnchorIds, undefined);
