...
test("history anchors cover unloaded and compacted turns only on the selected branch", () => {
  ...
  assert.equal(page.historyAnchors.at(-1).compaction, true);
  assert.equal(page.historyAnchors[0].compaction, undefined);
  assert.deepEqual(
    buildSessionContext(entries, "alternate", { tail: 1 }).historyAnchors.map(
      (anchor) => anchor.id,
    ),
    ["e0", "alternate"],
  );
...
test("a rail jump loads a contiguous older span through its target without crossing branches", () => {
  ...
  const page = buildSessionContext(entries, "e4950", {
    tail: 50,
    excludeLeaf: true,
    throughEntryId: "e100",
  });
  ...
  assert.throws(
    () =>
      buildSessionContext(entries, "e4950", {
        tail: 50,
        excludeLeaf: true,
        throughEntryId: "other",
      }),
    RangeError,
  );
});

test("full-branch previews stay bounded while only the latest messages load", () => {
  const entries = linearChain(20000);
  entries[0].message.content = "  Old\n\t**prompt**  😀 ";
  entries[2].message.content = "x".repeat(10000);
  const page = buildSessionContext(entries, "e19999", { tail: 4 });
  assert.equal(page.messages.length, 4);
  assert.equal(page.historyAnchors.length, 10000);
  assert.equal(page.historyAnchors[0].preview, "Old **prompt** 😀");
  assert.equal(page.historyAnchors[1].preview, "x".repeat(99) + "…");
  assert.equal(page.historyAnchors.at(-1).preview, "msg 19998");
  assert.ok(
    page.historyAnchors.every(
      (anchor) => Array.from(anchor.preview).length <= 100,
    ),
  );
  const branch = buildSessionContext(entries, "e3", { tail: 1 });
  assert.deepEqual(
    branch.historyAnchors.map((anchor) => anchor.id),
    ["e0", "e2"],
  );
});
