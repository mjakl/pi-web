...
test("tail keeps the latest assistant when the compaction boundary is outside the window", () => {
  const entries = [
    { id: "kept", parentId: null, type: "message", timestamp: "t1", message: { role: "user", content: "kept" } },
    { id: "filler1", parentId: "kept", type: "message", timestamp: "t2", message: { role: "assistant", content: "filler" } },
    { id: "filler2", parentId: "filler1", type: "message", timestamp: "t3", message: { role: "user", content: "filler" } },
    { id: "latest", parentId: "filler2", type: "message", timestamp: "t4", message: { role: "assistant", content: "latest response" } },
    { id: "compact", parentId: "latest", type: "compaction", timestamp: "t5", summary: "summary", firstKeptEntryId: "kept", tokensBefore: 10 },
    { id: "leaf", parentId: "compact", type: "session_info", timestamp: "t6", name: "renamed" },
  ];

  const context = buildSessionContext(entries, "leaf", { tail: 3 });

  assert.deepEqual(context.entryIds, ["latest", "compact"]);
});

test("session settings distinguish a missing thinking choice from explicit off", () => {
  const withoutChoice = linearChain(1);
  assert.equal(buildSessionContext(withoutChoice, "e0").thinkingLevel, null);

  const withOff = linearChain(1);
  withOff[0].parentId = "thinking";
  withOff.unshift({
    id: "thinking",
    parentId: null,
  ...
  assert.equal(buildSessionContext(withOff, "e0").thinkingLevel, "off");
});
...
test("buildSessionContext accepts a large tail and returns the whole chain", () => {
  ...
  // see app/api/sessions/[id]/route.ts and app/api/sessions/[id]/context/route.ts.
});
...
test("history anchors cover unloaded and compacted turns only on the selected branch", () => {
  const entries = linearChain(100);
  entries.push({ id: "compact", parentId: "e99", type: "compaction", timestamp: "t", summary: "summary", firstKeptEntryId: "e98", tokensBefore: 10 });
  entries.push({ id: "alternate", parentId: "e1", type: "message", timestamp: "t", message: { role: "user", content: "other branch" } });
  const page = buildSessionContext(entries, "compact", { tail: 3 });
  assert.deepEqual(page.entryIds, ["e98", "e99", "compact"]);
  assert.equal(page.historyAnchors.length, 51);
  assert.equal(page.historyAnchors[0].id, "e0");
  assert.equal(page.historyAnchors[0].timestamp, 1000);
  assert.equal(page.historyAnchors.at(-1).timestamp, undefined);
  assert.equal(page.historyAnchors.at(-1).id, "compact");
  assert.deepEqual(buildSessionContext(entries, "alternate", { tail: 1 }).historyAnchors.map(anchor => anchor.id), ["e0", "alternate"]);
  const full = buildSessionContext(entries, "compact", { tail: 200 });
  assert.equal(full.entryIds.length, 101);
  assert.equal(full.entryIds[0], "e0");
  assert.equal(full.hasMore, false);
});

test("a rail jump loads a contiguous older span through its target without crossing branches", () => {
  const entries = linearChain(5000);
  entries.push({ id: "other", parentId: "e1", type: "message", timestamp: "t", message: { role: "user", content: "other" } });
  const page = buildSessionContext(entries, "e4950", { tail: 50, excludeLeaf: true, throughEntryId: "e100" });
  assert.equal(page.entryIds.length, 4850);
  assert.equal(page.entryIds[0], "e100");
  assert.equal(page.entryIds.at(-1), "e4949");
  assert.equal(page.hasMore, true);
  assert.equal(page.historyAnchors, undefined);
  assert.throws(() => buildSessionContext(entries, "e4950", { tail: 50, excludeLeaf: true, throughEntryId: "other" }), RangeError);
});
