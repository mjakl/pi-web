...
test("forwards session_stopped and then closes the stream", async () => {
  let listener;
  const stream = createAgentEventStream(
  ...
  listener({ type: "session_stopped" });
  assert.deepEqual(decodeData(await readWithin(reader)), { type: "session_stopped" });
  assert.equal((await readWithin(reader)).done, true);
});
