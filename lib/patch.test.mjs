...
test("pairs unequal change runs without losing trailing removals", async () => {
  const { parseUnifiedPatch } = await loadSubject();
  const files = parseUnifiedPatch(`--- a/demo.ts
+++ b/demo.ts
@@ -1,3 +1,2 @@
...
`);
  ...
  assert.deepEqual(
    ...
    [
      {
        type: "line",
        left: { lineNo: 1, text: "old first", type: "removed" },
        right: { lineNo: 1, text: "new first", type: "added" },
      },
      {
        type: "line",
        left: { lineNo: 2, text: "old second", type: "removed" },
        right: { lineNo: null, text: "", type: "empty" },
      },
      {
        type: "line",
        left: { lineNo: 3, text: "tail", type: "context" },
        right: { lineNo: 2, text: "tail", type: "context" },
      },
    ],
  );
