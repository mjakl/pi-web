...
const piViaJiti = await jiti.import("@earendil-works/pi-coding-agent");
const { buildContextEntries } = piViaJiti;

test("jiti resolves the same host Pi as Node, not a second copy", async () => {
  const piViaNode = await import("@earendil-works/pi-coding-agent");
  assert.equal(piViaJiti.buildContextEntries, piViaNode.buildContextEntries);
});
