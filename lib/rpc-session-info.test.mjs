...
test("titles live rows like persisted rows: skill expansions collapse and long text is capped", (t) => {
  const previousRegistry = globalThis.__piSessions;
  const timestamp = "2026-08-12T01:02:04.000Z";
  const entry = (text) => ({ type: "message", id: "u1", parentId: null, timestamp, message: { role: "user", content: text } });
  const expandedSkill = [
  ...
  globalThis.__piSessions = new Map([
    ["skill-runtime", makeRuntimeSession({
      id: "skill-runtime",
      filePath: join(tmpdir(), "pi-web-missing-skill-session.jsonl"),
      running: true,
      entries: [entry(expandedSkill)],
    })],
    ["long-runtime", makeRuntimeSession({
      id: "long-runtime",
      filePath: join(tmpdir(), "pi-web-missing-long-session.jsonl"),
      running: true,
      entries: [entry("x".repeat(SESSION_TITLE_MAX_CHARS + 100))],
    })],
  ]);
  t.after(() => {
    globalThis.__piSessions = previousRegistry;
  ...
  const titles = Object.fromEntries(getRpcSessionInfos().map((info) => [info.id, info.firstMessage]));

  assert.equal(titles["skill-runtime"], "/skill:review check this branch");
  assert.equal(titles["long-runtime"].length, SESSION_TITLE_MAX_CHARS);
});
