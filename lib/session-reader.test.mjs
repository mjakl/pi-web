...
test("renders the SDK compaction-aware context with aligned entry IDs", () => {
  ...
  assert.equal(
    paged.messages[0].details.estimatedTokensAfter,
    10,
    "retained context can be outside the displayed page",
  );
  assert.deepEqual(
...
for (const customType of ["extension_debug", "compaction"]) {
  test(`preserves hidden ${customType} messages without navigation anchors`, () => {
    const entries = [
      userEntry("u1", null, "start"),
      {
        type: "custom_message",
...
test("remembered removed worktrees expose their surviving project root as the entry path", async (t) => {
  const dir = mkdtempSync(join(tmpdir(), "pi-web-removed-worktree-entry-"));
  setTestAgentDir(t, dir);
  const repo = join(dir, "repo");
  const removedWorktree = join(dir, "removed-worktree");
  mkdirSync(repo);
  writeFileSync(
    join(dir, "web-worktree-projects.json"),
    JSON.stringify({ [pathIdentityKey(removedWorktree)]: repo }),
  );
...
test("keeps forward and reverse session path caches in sync", async () => {
  ...
  assert.equal(
    globalThis.__piPathToSessionIdCache?.has(pathIdentityKey(filePath)),
    false,
  );
});
...
test("resolves a matching session header without a catalogue scan", async (t) => {
  ...
  assert.equal(
    globalThis.__piPathToSessionIdCache?.get(pathIdentityKey(filePath)),
    sessionId,
  );
...
test(
  ...
  async (t) => {
    ...
    const externalDir = mkdtempSync(
      join(tmpdir(), "pi-web-targeted-symlink-external-"),
    );
    ...
    symlinkSync(
      externalDir,
      join(agentDir, "sessions", "linked-project"),
      "dir",
    );
...
test("reparentChildSessions moves generic children up one level and keeps every other file intact", (t) => {
  ...
  const genericContent = [
    header("generic-child", parentPath),
    body,
    "x".repeat(256 * 1024),
    "",
  ].join("\n");
  ...
  const [genericHeader, ...genericRest] = readFileSync(
    genericChildPath,
    "utf8",
  ).split("\n");
  ...
  assert.equal(
    "parentSession" in
      JSON.parse(readFileSync(genericChildPath, "utf8").split("\n")[0]),
    false,
  );
});
