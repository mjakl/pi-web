...
test("a persisted snapshot projects every authoritative UI field together", () => {
  ...
  const snapshot = {
    ...
    leafId: "leaf-b",
    tree: [
      { id: "leaf-b", parentId: null, type: "message", label: "refreshed" },
    ],
...
test("persisted tool projection is side-effect-free for legacy, empty, and nonempty selections", () => {
  ...
  assert.equal(
    projectPersistedSnapshot(snapshot(undefined)).toolPreset,
    "default",
  );
  assert.equal(projectPersistedSnapshot(snapshot([])).toolPreset, "none");
  assert.equal(
    projectPersistedSnapshot(snapshot(["read", "grep", "find", "ls"]))
      .toolPreset,
    "read-only",
  );
  assert.equal(
    projectPersistedSnapshot(
      snapshot(["bash", "read", "edit", "write", "grep", "find", "ls"]),
    ).toolPreset,
    "full",
  );
});
...
test("a bounded refresh does not restore live messages that fell before its window", () => {
  const previous = Array.from({ length: 50 }, (_, index) =>
    user(`old ${index}`, index),
  );
  const live = Array.from({ length: 60 }, (_, index) =>
    assistant(`live ${index}`, 100 + index),
  );
...
test("accepted detail metadata fences inventory that was already in flight", () => {
  const inventoryA = sessionInfo("A", 10, {
    name: "A",
    messageCount: 1,
    firstMessage: "A",
  });
  const detailB = sessionInfo("B", 20, {
    name: "B",
    messageCount: 2,
    firstMessage: "B",
  });
  ...
  const inventoryFirst = acceptSelectedSessionMetadata(
    reconcileSelectedSessionInventory(
      { session: inventoryA, authority: null },
      inventoryA,
      1,
    ),
    detailB,
  ...
  const detailFirst = acceptSelectedSessionMetadata(
    { session: inventoryA, authority: null },
    detailB,
    1,
  );
  const delayedInventory = reconcileSelectedSessionInventory(
    detailFirst,
    inventoryA,
    1,
  );
...
test("inventory after accepted detail reconciles exact, changed, and missing fingerprints", () => {
  ...
  const accepted = acceptSelectedSessionMetadata(
    { session: sessionInfo("A", 10), authority: null },
    detailB,
    1,
  );

  const exact = reconcileSelectedSessionInventory(
    accepted,
    sessionInfo("B", 20),
    2,
  );
  ...
  const changed = reconcileSelectedSessionInventory(
    accepted,
    sessionInfo("C", 30),
    2,
  );
  ...
  const missingFingerprint = reconcileSelectedSessionInventory(
    accepted,
    {
  ...
  const laterOmission = reconcileSelectedSessionInventory(
    accepted,
    undefined,
    2,
  );
...
test("detail conflict creates no fence and lazy metadata remains fingerprint-scoped", () => {
  ...
  const inventory = reconcileSelectedSessionInventory(
    current,
    sessionInfo("C", 30),
    1,
  );
...
test("transient detail and selection changes do not inherit unrelated metadata", () => {
  ...
  const accepted = acceptSelectedSessionMetadata(
    { session: transient, authority: null },
    transient,
    1,
  );
  assert.equal(accepted.authority.fingerprint, null);
  assert.equal(
    reconcileSelectedSessionInventory(accepted, undefined, 1).session.name,
    "live",
  );
  ...
  const persisted = reconcileSelectedSessionInventory(
    accepted,
    sessionInfo("persisted", 10),
    2,
  );
  ...
  const other = {
    session: { ...sessionInfo("other", 5), id: "other" },
  ...
  assert.equal(
    reconcileSelectedSessionInventory(other, other.session, 2).authority,
    null,
  );
});
...
function acceptSnapshot(state, request, channel) {
  const latestOrder =
    channel === "canonical"
      ? state.latestCanonicalOrder
      : state.latestRefreshOrder;
  if (
    !canAcceptPersistedSnapshot(request, currentAuthority(state), latestOrder)
  )
    return false;
...
test("canonical and Refresh use successful acceptance in both start and completion orders", () => {
  for (const [firstChannel, secondChannel] of [
    ["canonical", "refresh"],
    ["refresh", "canonical"],
  ]) {
...
test("a failed canonical or Refresh request stamps no authority", () => {
  for (const failedChannel of ["canonical", "refresh"]) {
    ...
    const fallbackChannel =
      failedChannel === "canonical" ? "refresh" : "canonical";
    const fallback = startSnapshot(state, fallbackChannel);
    startSnapshot(state, failedChannel);

    assert.equal(
      acceptSnapshot(state, fallback, fallbackChannel),
      true,
      failedChannel,
    );
  }
...
test("canonical load and pagination preserve the first accepted baseline", () => {
  for (const pageStartsFirst of [false, true]) {
    for (const pageCompletesFirst of [false, true]) {
      const state = createAuthorityState();
      const first = pageStartsFirst
        ? startPagination(state)
        : startSnapshot(state, "canonical");
      const second = pageStartsFirst
        ? startSnapshot(state, "canonical")
        : startPagination(state);
      ...
      if (pageCompletesFirst) {
        ...
        assert.equal(
          acceptSnapshot(state, canonical, "canonical"),
          pageStartsFirst,
        );
      } else {
...
test("branch context and canonical load use transcript authority", () => {
  ...
  assert.equal(
    acceptSnapshot(canonicalWins, laterCanonical, "canonical"),
    true,
  );
...
test("local persisted mutations reject older full snapshots without rejecting transcript-only work", () => {
  for (const mutation of ["thinking", "tools", "model"]) {
    ...
    assert.equal(
      acceptSnapshot(state, laterRefresh, "refresh"),
      true,
      mutation,
    );
  }
...
test("only observed activity starts advance persisted replacement authority", () => {
  ...
  for (const boundary of [
    ...
    assert.equal(
      observedActivityEpochAfter(epoch, boundary),
      epoch + 1,
      boundary,
    );
    epoch += 1;
  ...
  for (const nonBoundary of [
    ...
    assert.equal(
      observedActivityEpochAfter(epoch, nonBoundary),
      epoch,
      nonBoundary,
    );
  }
...
test("branch context loading releases later settings before prompt submission", async () => {
  ...
  let contextStarted;
  let releaseContext;
  const contextHasStarted = new Promise((resolve) => {
    contextStarted = resolve;
  });
  const contextMayFinish = new Promise((resolve) => {
