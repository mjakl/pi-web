...
function addAll(state, ids) {
  return ids.reduce(
    (current, id) =>
      noticeReducer(current, { type: "add", notice: notice(id) }),
    state,
  );
}
...
test("queues new notices while one is already leaving instead of marking a second", () => {
  const leaving = noticeReducer(addAll(empty, ["1", "2"]), {
...
test("marks only the oldest notice that is not already leaving", () => {
  const once = noticeReducer(addAll(empty, ["1", "2", "3"]), {
    type: "mark_oldest_exiting",
  });
  const twice = noticeReducer(once, { type: "mark_oldest_exiting" });
  assert.deepEqual(shape(twice), {
    visible: [
      ["1", true],
      ["2", true],
      ["3", false],
  ...
  assert.deepEqual(
    noticeReducer(empty, { type: "mark_oldest_exiting" }),
    empty,
  );
});
...
test("removing with nothing queued just drops that notice", () => {
  const next = noticeReducer(addAll(empty, ["1", "2"]), {
    type: "remove",
    id: "1",
  });
