...
test("plain-text extension theme supports Pi required foregrounds", () => {
  const { context } = createBridge();

  assert.equal(context.theme.fg("text", "plain"), "plain");
  assert.equal(context.theme.fg("muted", "plain"), "plain");
});
...
test("timeouts, aborts, and dispose settle dialogs with their defaults", async () => {
  ...
  const aborted = context.input("Name", "placeholder", {
  ...
  assert.equal(
    await context.editor("Edit", "draft", { signal: AbortSignal.abort() }),
    undefined,
  );
