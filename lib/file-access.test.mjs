...
test("derives allowed roots from bounded session headers and ignores malformed files", async (t) => {
  ...
  fs.writeFileSync(
    path.join(projectDir, "object-cwd.jsonl"),
    `${JSON.stringify({
      type: "session",
      version: 3,
      id: "object-cwd",
  ...
  fs.writeFileSync(
    path.join(projectDir, "numeric-parent.jsonl"),
    `${JSON.stringify({
      type: "session",
      version: 3,
      id: "numeric-parent",
  ...
  assert.equal(
    [...roots].some((root) => root.includes("malformed")),
    false,
  );
});
...
test("authorizes a linked worktree session's main project root", async (t) => {
  const { createJiti } = await import("jiti");
  const { getAllowedFileRoots } = await createJiti(import.meta.url, {
    moduleCache: false,
  }).import("./file-access.ts");
  const agentDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "pi-web-worktree-roots-"),
  );
  ...
  fs.mkdirSync(projectDir, { recursive: true });
  fs.writeFileSync(
    path.join(projectDir, "worktree.jsonl"),
    `${JSON.stringify({
      type: "session",
      version: 3,
      id: "worktree-session",
...
test("authorizeDirectory denies before revealing existence and after symlinks resolve", async (t) => {
  const { createJiti } = await import("jiti");
  const { allowFileRoot, authorizeDirectory } = await createJiti(
    import.meta.url,
    {
      moduleCache: false,
    },
  ...
  const agentDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "pi-web-authorize-agent-"),
  );
  const base = fs.realpathSync(
    fs.mkdtempSync(path.join(os.tmpdir(), "pi-web-authorize-")),
  );
  const allowed = path.join(base, "allowed");
  const outside = path.join(base, "outside");
  ...
  assert.deepEqual(await authorizeDirectory(""), {
    status: 400,
    error: "cwd must be an absolute path",
  });
  assert.deepEqual(await authorizeDirectory("relative/dir"), {
    status: 400,
    error: "cwd must be an absolute path",
  });
  assert.deepEqual(await authorizeDirectory(path.join(outside, "missing")), {
    status: 403,
    error: "Access denied",
  });
  assert.deepEqual(await authorizeDirectory(path.join(allowed, "missing")), {
    status: 404,
    error: "Directory not found",
  });
  assert.deepEqual(await authorizeDirectory(file), {
    status: 400,
    error: "Not a directory",
  });
  assert.deepEqual(await authorizeDirectory(link), {
    status: 403,
    error: "Access denied",
  });
