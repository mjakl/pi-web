...
const { GET: getPlugins, POST: postPlugins } = await jiti.import(
  "../app/api/plugins/route.ts",
);
const { POST: installSkill } = await jiti.import(
  "../app/api/skills/install/route.ts",
);
const { POST: postProjectTrust } = await jiti.import(
...
async function createUntrustedProject(t) {
  const root = await realpath(
    await mkdtemp(join(tmpdir(), "pi-web-trust-boundaries-")),
  );
...
test("model discovery runs project extensions only after the project is trusted", async (t) => {
  ...
  const request = () =>
    new Request(`http://localhost/api/models?cwd=${encodeURIComponent(cwd)}`);
...
test("skill listing hides project skills and extensions until the project is trusted", async (t) => {
  const { cwd, agentDir, marker } = await createUntrustedProject(t);
  const hasProbe = (response) =>
    response.skills.some((skill) => skill.name === "project-probe");
...
test("plugin routes report project trust and refuse project changes until trusted", async (t) => {
  ...
  const listing = () =>
    getPlugins(
      new Request(
        `http://localhost/api/plugins?cwd=${encodeURIComponent(cwd)}`,
      ),
    );

  assert.equal((await (await listing()).json()).projectResourcesLoaded, false);
  const refused = await postPlugins(
...
test("trusting a project shuts down its restricted runtimes", async (t) => {
  ...
  const response = await postProjectTrust(
    new Request("http://localhost/api/project-trust", {
      method: "POST",
      headers: JSON_HEADERS,
