...
test("reduces tool execution progress to the displayed line", () => {
  assert.deepEqual(toClientAgentEvent({
    ...
    toolCallId: "call-workflow-1",
    toolName: "workflow",
    progress: "Running phase 2",
  });
});

test("tool execution progress carries no output when there is none to show", () => {
  assert.deepEqual(toClientAgentEvent({
    type: "tool_execution_update",
...
function projectedToolProgressBytes(totalLength) {
  const chunkSize = 64;
  let output = "";
  let bytes = 0;

  for (let offset = 0; offset < totalLength; offset += chunkSize) {
    output += `${"x".repeat(Math.min(chunkSize, totalLength - offset))}\n`;
    const projected = toClientAgentEvent({
      type: "tool_execution_update",
      toolCallId: "call-bash-1",
      toolName: "bash",
    ...
    assert.equal(Object.hasOwn(projected, "partialResult"), false);
    bytes += Buffer.byteLength(JSON.stringify(projected));
  }
...
test("serialized tool-progress traffic stays flat as command output accumulates", () => {
  const twoKiB = projectedToolProgressBytes(2 * 1024);
  const fourKiB = projectedToolProgressBytes(4 * 1024);
  assert.ok(fourKiB / twoKiB < 2.2, `expected near-linear growth, got ${fourKiB / twoKiB}`);
});
