...
export function sessionInfoFingerprint(
  ...
  return session.fileSize === undefined
    ? null
    : `${session.modified}\0${session.fileSize}`;
}
...
export function reconcileSelectedSessionInventory(
  ...
  const preserveMetadata =
    incomingFingerprint !== null && incomingFingerprint === fingerprint;
...
export function canAcceptInventoryResult(
  attempt: number,
...
export function observedActivityEpochAfter(
  epoch: number,
  boundary: string,
): number {
...
export function canAcceptPersistedSnapshot(
  ...
  return (
    request.order === latestRequestOrder &&
...
export function canAcceptPagination(
  ...
  current: Pick<PersistedAuthority, "sessionId" | "acceptedTranscriptOrder"> & {
    latestRefreshOrder: number;
  },
): boolean {
  return (
    request.sessionId === current.sessionId &&
...
function sameMessage(left: AgentMessage, right: AgentMessage): boolean {
  return (
    sameExactMessage(left, right) ||
    (left.role === "user" &&
      right.role === "user" &&
      userMessageKey(left) === userMessageKey(right))
  );
}
...
export function mergeTranscriptRefreshMessages(
  ...
  const previousBoundary = previousLast
    ? persisted.findLastIndex((message) =>
        sameExactMessage(message, previousLast),
      )
    : -1;
  ...
  let liveIndex =
    previousBoundary >= 0
      ...
      : liveTail.findIndex((liveMessage) =>
          refreshedTail.some((message) =>
            sameExactMessage(message, liveMessage),
          ),
        );
  if (liveIndex < 0) {
...
export function projectPersistedSnapshot<
  T extends {
