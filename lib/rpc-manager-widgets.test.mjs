...
function makeInner(overrides = {}) {
  return {
    ...
    sessionManager: { getCwd: () => process.cwd(), getSessionFile: () => undefined },
...
// Extensions receive the UI context and command actions through bindExtensions.
async function createContext(overrides = {}) {
  const events = [];
  let bindings;
  const wrapper = new AgentSessionWrapper(makeInner({
    bindExtensions: async (received) => { bindings = received; },
    ...overrides,
  }));
  wrapper.onEvent((event) => events.push(event));
  await wrapper.waitUntilReady();
  return { context: bindings.uiContext, commandActions: bindings.commandContextActions, events, wrapper };
}
...
test("keeps array widgets compatible while maintaining the state snapshot", async () => {
  const { context, events, wrapper } = await createContext();
...
test("clearing and replacing widgets disposes components and invalidates old callbacks", async () => {
  const { context, events, wrapper } = await createContext();
  let oldTui;
  let oldDisposed = 0;
...
test("command-context reload also clears factory widgets", async () => {
  const { commandActions, context, wrapper } = await createContext();
  let disposed = 0;
...
test("render failure does not clear a dispose-time recovery widget", async () => {
  const { context, wrapper } = await createContext();
...
test("ignores factory registrations after the wrapper is destroyed", async () => {
  const { context, events, wrapper } = await createContext();
  let factoryCalls = 0;
