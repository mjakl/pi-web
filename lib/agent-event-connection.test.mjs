...
function createHarness({
  ...
  const connection = new AgentEventConnection({
    createSource(sessionId, activate) {
...
test("explicit connections carry activation while passive maintenance does not", async () => {
  const explicit = createHarness();
  const ready = explicit.connection.ensureConnected("session-a", true);
  assert.equal(explicit.sources[0].activate, true);
  explicit.sources[0].open();
  explicit.sources[0].message({ type: "connected" });
  await ready;

  const passive = createHarness();
  passive.setDemand(true);
  passive.connection.maintain("session-a");
  assert.equal(passive.sources[0].activate, false);
...
test("same-ID replacement receives new events and terminal Stop on a fresh source", async () => {
  ...
  connection.close();
  const replacement = connection.ensureConnected("session-a");
  oldSource.message({ type: "agent_start" });
  sources[1].open();
  sources[1].message({ type: "connected" });
  await replacement;
  sources[1].message({ type: "agent_start" });
  sources[1].message({ type: "session_stopped" });
  await wait(10);

  assert.deepEqual(
    events.map((event) => event.type),
    ["connected", "connected", "agent_start", "session_stopped"],
  );
  assert.equal(oldSource.closeCount, 1);
...
test("session_stopped closes a ready source without reconnecting", async () => {
  const { connection, sources, events, setDemand } = createHarness();
  setDemand(true);
  const ready = connection.ensureConnected("session-a");
  sources[0].open();
  ...
  sources[0].message({ type: "session_stopped" });
  await wait(10);

  assert.deepEqual(
    events.map((event) => event.type),
    ["connected", "session_stopped"],
  );
  assert.equal(sources[0].closeCount, 1);
  assert.equal(sources.length, 1);
});
