...
test("missing convention-shaped folders use only recorded project associations", async (t) => {
  const root = await realpath(
    await mkdtemp(path.join(os.tmpdir(), "pi-web-missing-worktree-")),
  );
  t.after(() => rm(root, { recursive: true, force: true }));
  const repo = path.join(root, "repo");
  const missing = path.join(root, "repo-worktrees", "feature");
  await execFileAsync("git", ["init", repo]);
  const { resolveProject } = await loadSubject();

  assert.deepEqual(await resolveProject(missing), {
    projectRoot: missing,
    branch: null,
    isWorktree: false,
    isTopLevel: false,
  ...
  const { createJiti } = await import("jiti");
  const { pathIdentityKey } = await createJiti(import.meta.url).import(
    "./paths.ts",
  );
  await writeFile(
    path.join(agentDir, "web-worktree-projects.json"),
    JSON.stringify({ [pathIdentityKey(missing)]: repo }),
  );
...
test("bare repositories group peer worktrees and remember deleted folders across cache resets", async (t) => {
  const root = await realpath(
    await mkdtemp(path.join(os.tmpdir(), "pi-web-bare-")),
  );
  t.after(() => rm(root, { recursive: true, force: true }));
  ...
  const {
    ...
    assertWorkingDirectoryAvailable,
  } = await loadSubject();
  ...
  assert.deepEqual(
    (await listWorktrees(first)).map((w) => w.path),
    [first, second],
  );
  ...
  const { createJiti } = await import("jiti");
  const { attachSessionProjectInfo } = await createJiti(import.meta.url).import(
    "./session-reader.ts",
  );
  const [missing] = await attachSessionProjectInfo([
    { id: "old", cwd: second },
  ]);
  ...
  assert.equal(
    (await attachSessionProjectInfo([{ id: "old", cwd: second }]))[0]
      .cwdAvailable,
    true,
  );
