...
function session(id, projectRoot, modified) {
  return {
    ...
    cwd: projectRoot,
    projectRoot,
    projectKey: pathIdentityKey(projectRoot, "win32"),
    created: modified,
    modified,
    messageCount: 1,
    firstMessage: id,
...
test("recent projects enter through the newest session cwd rather than an inferred root", () => {
  const item = session("worktree", "/repo", "2026-08-13T00:00:00.000Z");
  item.cwd = "/repo-worktrees/feature";

  assert.equal(getRecentProjects([item])[0].root, "/repo");
  assert.equal(getRecentProjects([item])[0].cwd, "/repo-worktrees/feature");
});

test("recent projects use the surviving entry path for a removed worktree", () => {
  const item = session("removed-worktree", "/repo", "2026-08-13T00:00:00.000Z");
  item.cwd = "/repo-worktrees/removed";
  item.projectEntryPath = "/repo";

  assert.equal(getRecentProjects([item])[0].root, "/repo");
  assert.equal(getRecentProjects([item])[0].cwd, "/repo");
});
